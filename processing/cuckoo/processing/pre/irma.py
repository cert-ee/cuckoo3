...
class Irma(Processor):
    ...
    def init(self):
        self.url = cfg("irma", "url", subpkg="processing")
        self.timeout = cfg("irma", "timeout", subpkg="processing")
        self.scan = cfg("irma", "scan", subpkg="processing")

        self.force = cfg("irma", "force", subpkg="processing")
        self.submitter = cfg("irma", "submitter", subpkg="processing")
        self.rescan_time = cfg("irma", "rescan_time", subpkg="processing")
        self.probes = cfg("irma", "probes", subpkg="processing")
        self.min_suspicious = cfg("irma", "min_suspicious", subpkg="processing")
        self.min_malicious = cfg("irma", "min_malicious", subpkg="processing")
    ...
    def wait_scan_finished(self, scan_id, pause=3, verbose=False):
        ...
        while True:
            ...
            if status_name == "finished":
                ...
                status_name in ["flushed", "cancelling", "cancelled"] or status >= 1000
            ):  # >=1000 is for errors
    ...
    def _request_json(self, url, **kwargs):
        ...
        try:
            ...
            self.ctx.log.error(f"Unable to fetch IRMA results: {e.message}")
            return {}
    ...
    def _scan_file(self, filepath, force):
        ...
        self.ctx.log.debug(f"Scanning file: {filepath}")
    ...
    def _get_results(self, sha256):
        ...
        if result_id:
            results = self._request_json(
                urljoin(self.url, f"/api/v2/results/{result_id}")
            )
    ...
    def _handle_file_target(self):
        try:
            ...
            results = self._request_json(
                urljoin(self.url, f"/api/v2/files/{target.sha256}")
            )
            ...
            if not self.force and not self.scan and not results:
                ...
                if results.get("items") and self.rescan_time > 0:
                    ...
                    timestamp_last_scan = results.get("file_infos").get(
                        "timestamp_last_scan"
                    )
                    now = time.time()
            ...
            self.ctx.log.warning(f"Error while making IRMA request: {e}")
        return results
    ...
    def start(self):
        ...
        if info.get("probe_results"):
            ...
            return {}
        malicious_count = len(
            [r.get("results") for e, r in probe_avinfo.items() if r.get("results")]
        )
        ...
        if score:
            ...
            self.ctx.signature_tracker.add_signature(
                name="irma",
                score=score,
                short_description="IRMA sources report this target as malicious",

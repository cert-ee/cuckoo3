...
def _add_preconfigured_vpns(rooterctx, configuredsettings):
    ...
    for name, values in configuredsettings["vpns"].items():
        ...
        rooterctx.vpns.add_preconfigured_vpn(
            rooterctx,
            routing_table_id=values["routing_table"],
            interface_name=values["interface"],
            country=values["country"],
            name=name,
        )
...
def _add_autovpn_vpnpool(rooterctx, poolsettings):
    ...
    for name, settings in poolsettings["providers"].items():
        provider = VPNProvider(
            rooterctx=rooterctx, name=name, max_connections=settings["max_connections"]
        )
        for vpn in settings["vpns"]:
            country = vpn["country"]
            config = vpn["config_path"]
            log.debug(
                "Adding VPN pool VPN", provider=name, country=country, config=config
            )
...
def _start_rooter(
    ...
    if cfg("routing.yaml", "internet", "enabled", subpkg="node"):
        ...
        routing_table = cfg("routing.yaml", "internet", "routing_table", subpkg="node")
        log.debug(
            "Adding internet route", interface=interface, routing_table=routing_table
        )
        rooterctx.add_internet_route(routing_table=routing_table, interface=interface)

    if cfg("routing.yaml", "vpn", "preconfigured", "enabled", subpkg="node"):
        _add_preconfigured_vpns(
            rooterctx, cfg("routing.yaml", "vpn", "preconfigured", subpkg="node")
        )
    ...
    log.info("Starting Cuckoo rooter", group=socket_group, socket_path=socket_path)
...
@click.argument(
    "socket",
    type=click.Path(readable=False, dir_okay=False),
    default="/tmp/cuckoo3-rooter.sock",
    required=False,
)
...
@click.option(
    ...
    help="Path to the iptables(8) binary",
)
@click.option(
    "--ip",
    type=click.Path(exists=True),
    default="/sbin/ip",
    help="Path to the ip(8) binary",
)
...
@click.option(
    ...
    help="Path to the sudo(8) binary",
)
@click.option(
    "--openvpn",
    type=click.Path(),
    default="/usr/sbin/openvpn",
    help="Path to the OpenVPN(8) binary",
)
@click.option(
    "--print-command",
    is_flag=True,
...
def main(
    cwd, debug, socket, group, iptables, ip, sudo, sudo_path, openvpn, print_command
):
    ...
    if print_command:
        args = _make_rooter_command(cwd, debug, socket, group, iptables, ip, openvpn)
        print(" ".join(args))
        return

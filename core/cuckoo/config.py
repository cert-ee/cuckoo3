...
class Machinery(config.String):
    ...
    def _fill_cache(self):
        ...
        modules = enumerate_plugins("cuckoo.machineries.modules", globals(), Machinery)
...
typeloaders = {
    "cuckoo.yaml": {
        ...
        "tcpdump": {
            ...
            "path": config.FilePath(default_val="/usr/bin/tcpdump", must_exist=True),
        },
        "network_routing": {
        ...
        "platform": {"autotag": config.Boolean(default_val=False)},
        "state_control": {"cancel_unidentified": config.Boolean(default_val=False)},
        "processing": {
            "worker_amount": {
    ...
    "analysissettings.yaml": {
        "default": {
            ...
            "route": {
                ...
                "options": config.Dict(
                    element_class=config.String, default_val={}, allow_empty=True
                ),
            },
        },
        "platform": {
            "versions": config.Dict(
                config.List(config.String), default_val={"windows": ["10"]}
            ),
            "multi_platform": config.List(config.String, default_val=["windows"]),
            "fallback_platforms": config.List(config.String, default_val=["windows"]),
        },
        "limits": {

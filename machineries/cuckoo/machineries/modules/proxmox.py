...
class Proxmox(Machinery):
    ...
    def load_machines(self):
        ...
        for machine in self.list_machines():
            ...
            if machine.snapshot is None:
                log.debug(
                    f"no snapshot was given tyting to load first snapshot in list"
                )
    ...
    def restore_start(self, machine):
        ...
        if vm is None:
            raise errors.MachineNotFoundError(
                f"while restore_start of {machine.label}. Couldn't map label to ID"
            )
    ...
    def state(self, machine):
        ...
        prox = self._create_proxmoxer_connection()
        current_status = prox.nodes(vm.node_name).qemu(vm.vm_id).status.current.get()
        if current_status is None:
            raise errors.MachineryConnectionError(
                f"Error while getting status of {machine.label} response was None"
            )
        if current_status.get("qmpstatus") is None:
            raise errors.MachineUnexpectedStateError(
                f"Error While getting qmpstatus of {machine.label} qmpstatus is None"
            )
    ...
    def _create_proxmoxer_connection(self):
        ...
        try:
            ...
            log.error(
                f"Connecting to Proxmox server timedout"
                f"Try increasing the timeout value in <cwd>)/conf/machineries/proxmox.yaml"
            )
            raise e

        if tmp is None:
            raise errors.MachineryConnectionError(f"Couldn't connect to Proxmox.")
        return tmp

...
from .analyses import (
    Kinds as AnalysisKinds,
    AnalysisError,
    Settings,
    get_state,
    States as AnalysisStates,
    find_waiting_url_analysis,
    find_waiting_file_analysis,
)
...
from .strictcontainer import Analysis, SubmittedFile, SubmittedURL, Platform, Route
...
class OptionsReader:
    ...
    def read_value(cls, key, value):
        ...
        try:
            ...
            raise SubmissionError(f"Option {key!r} must be of type: {dtype.__name__}")
...
def find_extrpath_fileid(analysis_id, fileid):
    ...
    try:
        ...
        raise SubmissionError(f"Invalid filemap file. JSON decoding error: {e}")
    ...
    if not _is_correct_extrpath(extrpath):
        raise SubmissionError("Read filemap file entry is not a valid extrpath")
    ...
    def verify_settings(settings, nodeinfos, limits):
        ...
        if errs:
            raise SubmissionError(
                f"One or more limits were exceeded: {'. '.join(errs)}"
            )

        SettingsVerifier.verify_platforms(settings, nodeinfos, errs)
        if errs:
            raise SubmissionError(
                f"One or more invalid settings were specified: {'. '.join(errs)}"
            )

    @staticmethod
    def check_limits(settings, limits, error_list):
        if limits.get("max_timeout") and settings.timeout > limits.get("max_timeout"):
            error_list.append(
                f"Timeout exceeds maximum timeout of {limits.get('max_timeout')}"
            )

        if limits.get("max_priority") and settings.priority > limits.get(
            "max_priority"
        ):
            error_list.append(
                f"Priority exceeds maximum priority of {limits.get('max_priority')}"
            )

        if limits.get("max_platforms") and len(settings.platforms) > limits.get(
            "max_platforms"
        ):
            error_list.append(
                f"Amount of platforms exceeds maximum of {limits.get('max_platforms')}"
            )
    ...
    def verify_platforms(settings, nodeinfos, error_list):
        ...
        if settings.route and not settings.platforms:
            ...
            if not has_route:
                ...
                log.debug(
                    "Supporting node for route found",
                    node=info.name,
...
class SettingsHelper:
    ...
    def _check_browser(self, browser):
        ...
        if not find_in_lists(self._nodeinfos.all_machine_lists, tags=set([tag])):
            raise SubmissionError(
    ...
    def _read_command(self, command):
        ...
        if not isinstance(command, list):
            ...
            try:
                ...
                raise SubmissionError(f"Command contains invalid/incomplete parts: {e}")
    ...
    def add_platform(self, platform, os_version="", tags=[], settings={}):
        ...
        if not isinstance(os_version, str):
            raise SubmissionError(f"os_version must be a string. {os_version!r}")
    ...
    def set_route_type(self, route_type, platform_index=None):
        ...
        if platform_index is None:
            ...
            self._get_platform(platform_index).settings.route = Route(type=route_type)
    ...
    def set_route_option(self, option, platform_index=None):
        ...
        if platform_index is None:
            ...
            self._get_platform(platform_index).settings.route.set_options(**option)
...
class SettingsMaker:
    ...
    def _dump_modify_dt(self):
        return datetime.fromtimestamp(self._nodeinfos_dump_path.stat().st_mtime)
    ...
    def reload_nodeinfo_dump(self):
        ...
        if not self._nodeinfos_dump_path:
            raise SubmissionError("No node info dump path has been configured")

        if not self._nodeinfos_dump_path.is_file():
            raise SubmissionError("Configured node infos dump path does not exist. ")
    ...
    try:
        ...
        if MIN_SUBMIT_SIZE:
            if file_helper.size < MIN_SUBMIT_SIZE:
                raise SubmissionError(
                    f"File smaller than min_file_size: {MIN_SUBMIT_SIZE} bytes"
                )
        if MAX_SUBMIT_SIZE:
            if file_helper.size > MAX_SUBMIT_SIZE:
                raise SubmissionError(
                    f"File bigger than max_file_size: {MAX_SUBMIT_SIZE / (1024 * 1024 * 1024)} Gigabytes"
                )
